...
for human_path in tqdm(human_files_short):
    if face_detector(human_path):
        human_num += 1
human_accuracy_rate = human_num / 100

dog_num = 0
for dog_path in tqdm(dog_files_short):
    if face_detector(dog_path):
        dog_num += 1
dog_accuracy_rate = dog_num / 100

print(human_accuracy_rate, dog_accuracy_rate)
...
### TODO: 定义你的框架
Resnet50_model_1 = Sequential()
Resnet50_model_1.add(GlobalAveragePooling2D(input_shape=train_Resnet50.shape[1:]))
Resnet50_model_1.add(Dense(133, activation='softmax', input_shape=train_Resnet50.shape[0:]))
...
### TODO: 编译模型
Resnet50_model_1.compile(loss='categorical_crossentropy', optimizer='rmsprop', metrics=['accuracy'])
...
Resnet50_model_1.fit(train_Resnet50, train_targets, validation_data=(valid_Resnet50, valid_targets), 
...
Resnet50_model_1.load_weights('saved_models/weights.best.Resnet50.hdf5')
...
Resnet50_predictions = [np.argmax(Resnet50_model_1.predict(np.expand_dims(feature, axis=0))) for feature in test_Resnet50]
...
def Resnet50_predict(img_path):
    ...
    print(feature.shape)
    pred_vector = Resnet50_model_1.predict(feature)
    return dog_names[np.argmax(pred_vector)]

print(path_to_tensor('images/human1.jpg').shape)

Resnet50_predict('images/human1.jpg')
...
dog_names = [item[20:-1] for item in sorted(glob('dogImages/train/*/'))]
print('there are %s total dog categories' % len(dog_names))
...
def paths_to_tensor(images):
    list_of_tensors = [path_to_tensor(img_path) for img_path in tqdm(images)]
    return np.vstack(list_of_tensors)
...
Resnet50_model_1 = Sequential()
Resnet50_model_1.add(GlobalAveragePooling2D(input_shape=train_Resnet50.shape[1:]))
Resnet50_model_1.add(Dense(133, activation='softmax'))
...
checkpointer = ModelCheckpoint(filepath='saved_models/weights.best.Resnet50.hdf5', 
                               verbose=1, save_best_only=True)
Resnet50_model_1.fit(train_Resnet50, train_targets, 
...
def Resnet50_predict_breed(img_path):
    ...
    predicted_vector = Resnet50_model_1.predict(bottleneck_feature)
    return dog_names[np.argmax(predicted_vector)]


def MyDogPredict(img_path):
    img = cv2.imread(img_path)
    cv_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)
    plt.imshow(cv_rgb)
    plt.show()
...
## 自由地使用所需的代码单元数吧
MyDogPredict('images/human1.jpg')


# In[99]:


MyDogPredict('images/human2.jpeg')


# In[57]:


MyDogPredict('images/dog1.jpeg')


# In[58]:


MyDogPredict('images/dog2.jpeg')


# In[59]:


MyDogPredict('images/mao1.jpeg')


# In[60]:


MyDogPredict('images/mao2.jpeg')
